...
`);
    ...
    case 'gro': {
      ...
      const buildScript = [
        ...
        'RUN npm install -g @tjamescouch/gro @tjamescouch/agentpatch && npm cache clean --force',
        'USER node',
        'WORKDIR /home/node',
        'ENTRYPOINT ["gro"]',
        'CMD ["-i"]',
    ...
    case 'gtui': {
      ...
      const buildScript = [
        ...
        'FROM node:20-slim',
        'RUN apt-get update && apt-get install -y --no-install-recommends git && rm -rf /var/lib/apt/lists/*',
        'RUN npm install -g @tjamescouch/gro @tjamescouch/gtui @tjamescouch/agentpatch && npm cache clean --force',
        'USER node',
        'WORKDIR /home/node',
        'ENTRYPOINT ["gtui"]',
        'CONTAINERFILE_EOF',
